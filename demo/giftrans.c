...
** It has also been compiled on AIX 3.2 and Solaris 2.3.
...
**	Will be fixed in 2.0 (if ever)
**	-D option may output changed data instead of original data, use
**	with caution, best only with then -L option.
**
** Version history
**
** Version 1.12.4 - 12 March 2010
**      adjust patch of "rgb.txt" and comments in rgb.txt starting '!'
**        -- Dirk Jagdmann <doj@cubic.org>,
**
** Version 1.12.3 - 1.5.96
**	Precedence Errors for roll (>>) operations.
**	Problem resulted in incorrect Disposal Method, and Color Resolution
**	  -- Anthony Thyssen <anthony@cit.gu.edu.au>
**	Extension Error (for gif's output by ImageMagick)
**        -- cristy@magick.es.dupont.com
**
...
**	Incorporated dumpcomment by omerzu@quantum.de (Thomas Omerzu)
**	Outputs original version if newer than GIF89a.
**
** Version 1.11.2 - 14.12.94
**	Incorporated OS/2 port by k.rusch@ieee.org (Klaus Johannes Rusch)
...
**	k.rusch@ieee.org (Klaus Johannes Rusch)
**	omerzu@quantum.de (Thomas Omerzu)
**	Willem.Vermin@sara.nl (Willem Vermin)
...
char copyright[] = "@(#)Copyright (C) 1994 by Andreas Ley (ley@rz.uni-karlsruhe.de) - dojfix";
char sccsid[] = "@(#)GIFtrans v1.12.4 - Transpose GIF files - with various fixes";

#ifndef RGBTXT
#ifdef LINUX
#define	RGBTXT	"/usr/share/X11/rgb.txt"
#else
#ifdef X11
		...
		if ((src=fopen(rgb,"r"))!=NULL) {
			...
			while (fgets(line,sizeof(line),src)) {
				...
				*ptr++='\0';
				...
				for (;strchr(" \t",*ptr);ptr++);
				nptr=ptr;
				for (ptr=strchr(ptr,'\0');ptr>nptr&&strchr(" \t\r\n",*(ptr-1));ptr--);
				*ptr='\0';
...
#endif /* MSDOS */
#ifdef OS2
	if(oname==NULL&&!freopen("", "wb", stdout)) {
#endif /* OS2 */
...
#endif /* MSDOS */
#ifdef OS2
	if(optind==argc&&!freopen("", "rb", stdin)) {
#endif /* OS2 */
